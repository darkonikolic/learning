...
GSD commands that connect to executable spec thinking:

| Command | What it does |
|---------|-------------|
| `/gsd:plan-phase --tdd` | Generates task structure that follows spec-driven TDD sequence |
| `/gsd:validate-phase N` | Maps implementation coverage to SPEC acceptance criteria; reports gaps |
| `/gsd:add-tests` | Generates test stubs from UAT criteria in the phase |
| `/gsd:verify-work` | Runs acceptance items against current behavior interactively |

`/gsd:validate-phase` is the most useful for executable spec work. It reads the SPEC acceptance criteria and the test files and produces a coverage table showing which criteria have corresponding tests and which do not. Gaps in coverage are either:
...
**Running /gsd:validate-phase for GET /tasks:**

```
/gsd:validate-phase 2
```

Read the output table. For each acceptance criterion, it should report a corresponding test. If any criterion shows "no coverage", you have one of two problems:
...
- [ ] /gsd:validate-phase is in my post-execute workflow — I run it and read the coverage table

...
## Step 7: run /gsd:validate-phase

```
/gsd:validate-phase 2
```

(Substitute the phase number for GET /tasks in your .planning/ setup.)

Read the output. It produces a table showing acceptance criteria and their test coverage. Expected output structure:

```
Phase 2 validation report:
  Coverage gaps: 2
  
  Criterion: "GET /tasks returns 200 with JSON array"
  Status: COVERED (TestGetTasks_Returns200)
  
  Criterion: "Empty list returns []"
  Status: NOT COVERED — no test found
  
  Criterion: "Tasks in creation order"
  Status: COVERED (TestGetTasks_CreationOrder)
```
...
If /gsd:validate-phase is not available (GSD not set up for this phase): run test coverage manually:

```bash
go test ./... -v | grep -E "^(=== RUN|--- PASS|--- FAIL)"
```

Count tests against acceptance criteria. Note any criterion with no corresponding test.
...
- [ ] /gsd:validate-phase run and output read — coverage gaps noted
- [ ] Missing tests written for any gaps found by /gsd:validate-phase

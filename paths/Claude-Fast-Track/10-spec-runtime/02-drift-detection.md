...
**After /gsd:execute-phase with --gaps-only**
...
**STATE.md says "completed" but /gsd:verify-work fails items**
...
**Coverage gaps from /gsd:validate-phase**
...
## GSD commands for drift detection

| Command | What it checks | When to use |
|---------|---------------|-------------|
| `/gsd:verify-work` | Acceptance items vs current behavior | After every execute session |
| `/gsd:validate-phase N` | Test coverage vs SPEC acceptance criteria | After execute, before ship |
| `/gsd:secure-phase N` | Security threat mitigations vs implementation | Before ship on any API |
| `/gsd:code-review` | Code quality vs PLAN.md intent | Before PR |

`/gsd:verify-work` is conversational — it walks through each acceptance item and asks you to confirm the behavior. Use it for quick post-execute sanity checks.

`/gsd:validate-phase N` is systematic — it reads the phase artifacts and produces a coverage table. Use it for thorough pre-ship verification.

Do not skip both. They check different things. `/gsd:verify-work` checks runtime behavior. `/gsd:validate-phase` checks artifact coverage.
...
Post-execute state after `/gsd:execute-phase 3` (PATCH /tasks/:id/complete):
...
- [ ] /gsd:verify-work is in my post-execute routine
- [ ] /gsd:validate-phase is in my pre-ship routine

...
**Spec writing for complex domains.** Writing a feature SPEC in `docs/specs/` for an endpoint with cross-cutting concerns (validation + auth + rate limiting + pagination + error format) requires holding many constraints simultaneously. Opus's reasoning depth handles this better.
...
**Standard frame → plan flow.** When the phase intent is clear (you can describe it in two sentences), frame and plan with Sonnet produce adequate quality. The planning structure (waves and tasks in `docs/plans/<phase>-plan.md`) does not require Opus-level reasoning when the requirements are already clear.
...
If you can describe the feature in two sentences with no caveats, use Sonnet. If you find yourself writing a paragraph with "but also" and "depends on" and "unless", use Opus — or tighten the feature SPEC first to resolve the ambiguity before the model engages with it.
...
**Per-agent override in config:** Open `.claude/settings.json` and agent definitions under `.claude/agents/` and find the model profile section. Each agent type has a model field. Change the value to override the default. See the lab in this module.
...
| spec (in `docs/specs/`) | Opus | Ambiguity scoring and acceptance criteria require deep reasoning |
| frame | Sonnet | Phase intent already clear; adaptive questioning at this depth |
| plan | Sonnet | Translating clear requirements into phase plan wave structure |
| execute | Sonnet | Executing defined tasks: writing handler.go, store.go, tests |
| verify | Sonnet | Running checks against defined acceptance criteria |
...
For task-api's three endpoints (POST /tasks, GET /tasks, PATCH /tasks/:id/complete), frame and plan on Sonnet produce adequate plans when SPECs are clear.
...
- [ ] I can find where model assignments live in `.claude/settings.json` and agent definitions under `.claude/agents/`.

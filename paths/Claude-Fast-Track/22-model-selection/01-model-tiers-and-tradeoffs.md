...
| Sonnet | claude-sonnet-4-6 | Balanced. Standard code generation, plan execution, typical GSD workflow. Default for most work. |
...
## GSD's model profile system

GSD assigns different models to different agent types based on what each agent does:

- **Research agents** (plan-phase with `--research`, spec writing) — default to Opus because they synthesize ambiguous inputs
- **Executor agents** (execute-phase task execution, code writing) — default to Sonnet because they implement clear specifications
- **Reviewer agents** (code-review, secure-phase) — default to Opus for adversarial quality
- **Summary/classification agents** (milestone-summary, status checks) — can use Haiku

These defaults are stored in `.planning/config.json` under the model profile section. You can override them per agent type — see the lab in module 22.
...
You do not control extended thinking directly in most GSD commands — it activates based on the model profile assignment. If you are using Opus for a task and want the reasoning trace visible, that is the default behavior.
...
- [ ] I know which GSD agent types default to Opus and which default to Sonnet.

...
This is the end-to-end integration of everything in this track. You will complete Phase 2 (GET /tasks) and Phase 3 (PATCH /tasks/:id/complete) for task-api using the full Claude Code spec-driven development workflow. No step-by-step handholding — use the reference map at the bottom of this file when you get stuck.
...
Ask Claude to run a code review:

```
Run a code review on the Phase 2 changes.
```
...
No SPEC exists yet for Phase 3. Write it from scratch before running any implementation commands.
...
Run the test coverage check and map test names to acceptance criteria:

```bash
go test ./... -v | grep -E "^(=== RUN|--- PASS|--- FAIL)"
```

Write tests for any acceptance criteria with no corresponding test.

Run a security review:

```
Run a security review for Phase 3.
```
...
```
Run a code review on the Phase 3 changes.
```
...
| SPEC and code diverged | 06 | 03-spec-drift-and-repair.md |
| Lab for GET /tasks SPEC | 06 | 04-lab-write-spec-for-feature.md |
| Executable verification | 07 | 01-executable-spec-thinking.md |
| Drift detection procedure | 07 | 02-drift-detection.md |
| Spec audit lab | 07 | 03-lab-spec-vs-implementation-audit.md |
| Context window polluted | 02 | 03-context-ownership.md |
| Need to add a rule | 04 | 04-rules-and-skills-authoring.md |
| Agent produced wrong output | 05 | 08-trust-but-verify.md |
| Two agents conflicting | 05 | 06-partial-failure-and-recovery.md |
...
The track is a permanent reference. When you encounter Claude Code behaviors not explained here, find the nearest module and add a note. The reference map grows as the system expands.

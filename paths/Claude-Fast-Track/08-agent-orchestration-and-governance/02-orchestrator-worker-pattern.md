...
The orchestrator-worker pattern is the core architecture of GSD execute-phase and any multi-agent system with dependent tasks. Understanding the vocabulary — fan-out, fan-in, DAG, wave — is prerequisite to reading PLAN.md, understanding execute-phase behavior, and debugging multi-agent failures.
...
**Worker (executor agent):** the agent that executes one specific task. It receives a task description and context from the orchestrator. It produces a result. It does not understand the broader workflow. In GSD, these are the executor agents spawned per task.

**Supervisor:** a variant of the orchestrator that also monitors worker health and intervenes when a worker stalls or produces invalid output. GSD execute-phase functions as a supervisor: it checks agent output and can spawn correction agents.
...
GSD execute-phase executes one wave at a time:
...
GSD execute-phase implements a version of this: after each executor completes, it checks STATE.md and git commits. If the task produced no commit (nothing changed), the task may have failed silently. The orchestrator can detect this and flag it.
...
- [ ] I know that GSD wave execution = one fan-out/fan-in cycle per wave.

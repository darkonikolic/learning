...
In Claude Code + GSD: harder to measure directly without tooling. Approximate metrics: pass rate on first execute-phase run, frequency of --gaps-only retries, frequency of manual corrections.

---

## Distributed tracing terminology

**Trace:** a record of one complete workflow execution. Example: one `/gsd:execute-phase` run from start to finish.
...
**Correlation ID:** an identifier that links all spans in one trace. In GSD, this is implicitly the phase identifier — all activity for phase 01-01 is linked by that identifier.
...
## Observability tools in Claude Code and GSD
...
| `/gsd:stats` | Phase statistics: tasks, waves, completion rate |
| `/gsd:health` | Planning directory health, inconsistencies |
...
- [ ] I know GSD's audit trail components: git commits, STATE.md, REQUIREMENTS.md.
